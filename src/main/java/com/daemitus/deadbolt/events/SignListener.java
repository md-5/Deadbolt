...
public class SignListener extends org.bukkit.event.block.BlockListener {
    ...
    public void onSignChange(SignChangeEvent event) {
        ...
        if (Config.hasPermission(player, Perm.user_color))
            for (int i = 0; i < 4; i++)
                lines[i] = Config.replaceColor(lines[i]);

        String ident = Config.stripColor(lines[0]);
        ...
        if (block.getType().equals(Material.WALL_SIGN)) {
            ...
            sign.setLine(0, isPrivate ? Config.default_private : Config.default_moreusers);
            sign.update();
        ...
        switch (result) {
            ...
            case SUCCESS:
                if (isPrivate) {
                    String owner = Config.stripColor(lines[1]);
                    if (owner.isEmpty())
